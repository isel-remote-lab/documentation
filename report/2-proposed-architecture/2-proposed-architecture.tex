...
A hierarchical system was designed in the following structure: \textit{Student → Professor → Administrator}. The administrator role inherits from the other roles. Professor from student, and student as the base role. A user can only have one role assigned, which means that this system is design as a simple hierarchical role and not a \acf{rbac}.
